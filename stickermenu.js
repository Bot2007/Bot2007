...
const ftoko = { key: { fromMe: false, participant: `0@s.whatsapp.net`, remoteJid: `status@broadcast` }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync(`./src/TaufikBot.png`)}, "title": "𝐒𝐓𝐈𝐂𝐊𝐄𝐑 𝐌𝐄𝐍𝐔 𝐓𝐀𝐔𝐅𝐈𝐊 𝐁𝐎𝐓", "description": "Taufik Bot", "currencyCode": "MYR", "priceAmount1000": "999999999", "retailerId": "TaufikBot", "productImageCount": 1}, "businessOwnerJid": `601173093564@s.whatsapp.net`}}}
...
`.trim(), null, {contextInfo: {
  ...
  'sticker': 'Sticker Menu',
    ...
    let before = conn.stickermenu.before || `
...
┗━━━━━━━━━━━━━━━━
%readmore`
    let header = conn.stickermenu.header || '┏━━°❀❬ %category ❭❀°━━┓'
    let body   = conn.stickermenu.body   || '┣➥ %cmd%islimit'
    let footer = conn.stickermenu.footer || '┗━━━━━━━━━━━━━━━━\n'
    let after  = conn.stickermenu.after  || (conn.user.jid == global.conn.user.jid ? '' : `Bot Asli :\nwa.me/${global.conn.user.jid.split`@`[0]}`) + `\n*%npmname@^%version*\n\`\`\`\%npmdesc\`\`\``
    let _text  = before + '\n'
    ...
    _text += after
    text =  typeof conn.stickermenu == 'string' ? conn.stickermenu : typeof conn.stickermenu == 'object' ? _text : ''
    let replace = {
      '%': '%',
      p: _p, uptime,
