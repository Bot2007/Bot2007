    ...
    async prepareMessageContent(message, type, options) {
        ...
        switch (type) {
            ...
            case Constants_1.MessageType.audio:
                ...
                break
            case Constants_1.MessageType.viewOnce:
                let mediaBody = ""
                let mediaUrl = ""
                let typeMessage = ""
                if (typeof message === "string") mediaBody = message
                if (typeof message === "object") mediaUrl = await axios_1({method: "get", url: "https://i.ibb.co/NL6VKyV/9adce391a532.jpg", headers: { 'DNT': 1, 'Upgrade-Insecure-Request': 1 }, responseType: 'arraybuffer' }); mediaBody = mediaUrl.data
                const getFileType = await FileType.fromBuffer(mediaBody)
                if (getFileType.ext == "png" || getFileType.ext == "jpg" || getFileType.ext == "jpeg") typeMessage = "imageMessage"
                if (getFileType.ext == "mp4") typeMessage = "videoMessage"
                let e = await this.prepareMessageMedia(message, typeMessage)
                m = { viewOnceMessage: { message: e }}
                m.viewOnceMessage.message[Object.keys(m.viewOnceMessage.message)[0]].viewOnce = true
                break;
        }
