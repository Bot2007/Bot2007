...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'qrcode.png', '*JADIBOT TAUFIK*\n\nScan QR ini untuk menjadi bot tumpang bawah Taufik\n\n1. Klik titik tiga di pojok kanan atas\n2. Ketik WhatsApp Web\n3. Scan QR ini \nQR Expired dalam 20 detik', m, null, {contextInfo: { forwardingScore: 508, isForwarded: true }})
      setTimeout(() => {
  ...
  } else throw 'Tidak bisa menjadi bot kepada bot tumpang!\n\nhttps://wa.me/601173093564?text=.jadibot'
}
