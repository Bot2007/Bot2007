...
const ftoko = { key: { fromMe: false, participant: `0@s.whatsapp.net`, remoteJid: `status@broadcast` }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync(`./src/TaufikBot.png`)}, "title": "𝐀𝐍𝐎𝐍𝐘𝐌𝐎𝐔𝐒 𝐌𝐄𝐍𝐔 𝐓𝐀𝐔𝐅𝐈𝐊 𝐁𝐎𝐓", "description": "Taufik Bot", "currencyCode": "MYR", "priceAmount1000": "999999999", "retailerId": "TaufikBot", "productImageCount": 1}, "businessOwnerJid": `601173093564@s.whatsapp.net`}}}
...
`.trim(), null, {contextInfo: {
  ...
  'anonymous': 'Anonymous Menu',
    ...
    conn.anonymousmenu = conn.anonymousmenu ? conn.anonymousmenu : {}
    let before = conn.anonymousmenu.before || `
...
┗━━━━━━━━━━━━━━━━
%readmore`
    let header = conn.anonymousmenu.header || '┏━━°❀❬ %category ❭❀°━━┓'
    let body   = conn.anonymousmenu.body   || '┣➥ %cmd%islimit'
    let footer = conn.anonymousmenu.footer || '┗━━━━━━━━━━━━━━━━\n'
    let after  = conn.anonymousmenu.after  || (conn.user.jid == global.conn.user.jid ? '' : `Bot Asli :\nwa.me/${global.conn.user.jid.split`@`[0]}`) + `\n*%npmname@^%version*\n\`\`\`\%npmdesc\`\`\``
    let _text  = before + '\n'
    ...
    _text += after
    text =  typeof conn.anonymousmenu == 'string' ? conn.anonymousmenu : typeof conn.anonymousmenu == 'object' ? _text : ''
    let replace = {
      '%': '%',
      p: _p, uptime,
