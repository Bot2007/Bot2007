...
const ftoko = { key: { fromMe: false, participant: `0@s.whatsapp.net`, remoteJid: `status@broadcast` }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync(`./src/TaufikBot.png`)}, "title": "𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃𝐄𝐑 𝐌𝐄𝐍𝐔 𝐓𝐀𝐔𝐅𝐈𝐊 𝐁𝐎𝐓", "description": "Taufik Bot", "currencyCode": "MYR", "priceAmount1000": "999999999", "retailerId": "TaufikBot", "productImageCount": 1}, "businessOwnerJid": `601173093564@s.whatsapp.net`}}}
...
`.trim(), null, {contextInfo: {
  ...
  'Kerang': 'Kerang Menu',
    ...
    conn.downloadermenu = conn.downloadermenu ? conn.downloadermenu : {}
    let before = conn.downloadermenu.before || `
...
┗━━━━━━━━━━━━━━━━
%readmore`
    let header = conn.downloadermenu.header || '┏━━°❀❬ %category ❭❀°━━┓'
    let body   = conn.downloadermenu.body   || '┣➥ %cmd%islimit'
    let footer = conn.downloadermenu.footer || '┗━━━━━━━━━━━━━━━━\n'
    let after  = conn.downloadermenu.after  || (conn.user.jid == global.conn.user.jid ? '' : `Bot Asli :\nwa.me/${global.conn.user.jid.split`@`[0]}`) + `\n*%npmname@^%version*\n\`\`\`\%npmdesc\`\`\``
    let _text  = before + '\n'
    ...
    _text += after
    text =  typeof conn.downloadermenu == 'string' ? conn.downloadermenu : typeof conn.downloadermenu == 'object' ? _text : ''
    let replace = {
      '%': '%',
      p: _p, uptime,
