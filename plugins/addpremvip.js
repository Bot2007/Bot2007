let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (global.prems.includes(who.split`@`[0]) && global.DATABASE.data.chats[m.chat].premium >= 0) return m.reply(`${conn.getName(who)} melanggani premium berluput. Ketik ${usedPrefix}cekpremium untuk melihat kapan premium ${conn.getName(who)} luput.`.trim())
