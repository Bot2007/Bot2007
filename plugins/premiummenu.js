...
const ftoko = { key: { fromMe: false, participant: `0@s.whatsapp.net`, remoteJid: `status@broadcast` }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync(`./src/TaufikBot.png`)}, "title": "𝐏𝐑𝐄𝐌𝐈𝐔𝐌 𝐌𝐄𝐍𝐔 𝐓𝐀𝐔𝐅𝐈𝐊 𝐁𝐎𝐓", "description": "Taufik Bot", "currencyCode": "MYR", "priceAmount1000": "999999999", "retailerId": "TaufikBot", "productImageCount": 1}, "businessOwnerJid": `601173093564@s.whatsapp.net`}}}
...
`.trim(), null, {contextInfo: {
  ...
  'Premium': 'Premium Menu',
    ...
    conn.premiummenu = conn.premiummenu ? conn.premiummenu : {}
    let before = conn.premiummenu.before || `
...
┗━━━━━━━━━━━━━━━━
%readmore`
    let header = conn.premiummenu.header || '┏━━°❀❬ %category ❭❀°━━┓'
    let body   = conn.premiummenu.body   || '┣➥ %cmd%islimit'
    let footer = conn.premiummenu.footer || '┗━━━━━━━━━━━━━━━━\n'
    let after  = conn.premiummenu.after  || (conn.user.jid == global.conn.user.jid ? '' : `Bot Asli :\nwa.me/${global.conn.user.jid.split`@`[0]}`) + `\n*%npmname@^%version*\n\`\`\`\%npmdesc\`\`\``
    let _text  = before + '\n'
    ...
    _text += after
    text =  typeof conn.premiummenu == 'string' ? conn.premiummenu : typeof conn.premiummenu == 'object' ? _text : ''
    let replace = {
      '%': '%',
      p: _p, uptime,
