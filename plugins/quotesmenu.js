...
const ftoko = { key: { fromMe: false, participant: `0@s.whatsapp.net`, remoteJid: `status@broadcast` }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync(`./src/TaufikBot.png`)}, "title": "𝐐𝐔𝐎𝐓𝐄𝐒 𝐌𝐄𝐍𝐔 𝐓𝐀𝐔𝐅𝐈𝐊 𝐁𝐎𝐓", "description": "Taufik Bot", "currencyCode": "MYR", "priceAmount1000": "999999999", "retailerId": "TaufikBot", "productImageCount": 1}, "businessOwnerJid": `601173093564@s.whatsapp.net`}}}
...
`.trim(), null, {contextInfo: {
  ...
  'quotes': 'Quotes Menu',
    ...
    conn.quotesmenu = conn.quotesmenu ? conn.quotesmenu : {}
    let before = conn.quotesmenu.before || `
...
┗━━━━━━━━━━━━━━━━
%readmore`
    let header = conn.quotesmenu.header || '┏━━°❀❬ %category ❭❀°━━┓'
    let body   = conn.quotesmenu.body   || '┣➥ %cmd%islimit'
    let footer = conn.quotesmenu.footer || '┗━━━━━━━━━━━━━━━━\n'
    let after  = conn.quotesmenu.after  || (conn.user.jid == global.conn.user.jid ? '' : `Bot Asli :\nwa.me/${global.conn.user.jid.split`@`[0]}`) + `\n*%npmname@^%version*\n\`\`\`\%npmdesc\`\`\``
    let _text  = before + '\n'
    ...
    _text += after
    text =  typeof conn.quotesmenu == 'string' ? conn.quotesmenu : typeof conn.quotesmenu == 'object' ? _text : ''
    let replace = {
      '%': '%',
      p: _p, uptime,
