...
const ftoko = { key: { fromMe: false, participant: `0@s.whatsapp.net`, remoteJid: `status@broadcast` }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync(`./src/TaufikBot.png`)}, "title": "𝐇𝐎𝐒𝐓 𝐌𝐄𝐍𝐔 𝐓𝐀𝐔𝐅𝐈𝐊 𝐁𝐎𝐓", "description": "Taufik Bot", "currencyCode": "MYR", "priceAmount1000": "999999999", "retailerId": "TaufikBot", "productImageCount": 1}, "businessOwnerJid": `601173093564@s.whatsapp.net`}}}
...
`.trim(), null, {contextInfo: {
  ...
  'host': 'Host Menu',
    ...
    conn.hostmenu = conn.hostmenu ? conn.hostmenu : {}
    let before = conn.hostmenu.before || `
...
┗━━━━━━━━━━━━━━━━
%readmore`
    let header = conn.hostmenu.header || '┏━━°❀❬ %category ❭❀°━━┓'
    let body   = conn.hostmenu.body   || '┣➥ %cmd%islimit'
    let footer = conn.hostmenu.footer || '┗━━━━━━━━━━━━━━━━\n'
    let after  = conn.hostmenu.after  || (conn.user.jid == global.conn.user.jid ? '' : `Bot Asli :\nwa.me/${global.conn.user.jid.split`@`[0]}`) + `\n*%npmname@^%version*\n\`\`\`\%npmdesc\`\`\``
    let _text  = before + '\n'
    ...
    _text += after
    text =  typeof conn.hostmenu == 'string' ? conn.hostmenu : typeof conn.hostmenu == 'object' ? _text : ''
    let replace = {
      '%': '%',
      p: _p, uptime,
...
handler.help = ['hostmenu']
handler.tags = ['jenismenu']
handler.command = /^(hostmenu)$/i
handler.owner = false
handler.mods = false
