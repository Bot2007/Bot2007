...
const ftoko = { key: { fromMe: false, participant: `0@s.whatsapp.net`, remoteJid: `status@broadcast` }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync(`./src/TaufikBot.png`)}, "title": "𝐀𝐃𝐕𝐀𝐍𝐂𝐄𝐃 𝐌𝐄𝐍𝐔 𝐓𝐀𝐔𝐅𝐈𝐊 𝐁𝐎𝐓", "description": "Taufik Bot", "currencyCode": "MYR", "priceAmount1000": "999999999", "retailerId": "TaufikBot", "productImageCount": 1}, "businessOwnerJid": `601173093564@s.whatsapp.net`}}}
...
`.trim(), null, {contextInfo: {
  ...
  'advanced': 'Advanced Menu',
    ...
    conn.advancedmenu = conn.advancedmenu ? conn.advancedmenu : {}
    let before = conn.advancedmenu.before || `
...
┗━━━━━━━━━━━━━━━━
%readmore`
    let header = conn.advancedmenu.header || '┏━━°❀❬ %category ❭❀°━━┓'
    let body   = conn.advancedmenu.body   || '┣➥ %cmd%islimit'
    let footer = conn.advancedmenu.footer || '┗━━━━━━━━━━━━━━━━\n'
    let after  = conn.advancedmenu.after  || (conn.user.jid == global.conn.user.jid ? '' : `Bot Asli :\nwa.me/${global.conn.user.jid.split`@`[0]}`) + `\n*%npmname@^%version*\n\`\`\`\%npmdesc\`\`\``
    let _text  = before + '\n'
    ...
    _text += after
    text =  typeof conn.advancedmenu == 'string' ? conn.advancedmenu : typeof conn.advancedmenu == 'object' ? _text : ''
    let replace = {
      '%': '%',
      p: _p, uptime,
...
handler.help = ['advancedmenu']
handler.tags = ['jenismenu']
handler.command = /^(advancedmenu)$/i
handler.owner = false
handler.mods = false
