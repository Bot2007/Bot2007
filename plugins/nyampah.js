let handler = async (m, { conn }) => {
    let __timers = (new Date - global.DATABASE._data.users[m.sender].lastnyampah)
    let _timers = (43200000 - __timers)
    let timers = clockString(_timers) 
    let user = global.DATABASE._data.users[m.sender]
    if (new Date - global.DATABASE._data.users[m.sender].lastnyampah > 43200000) {
    conn.reply(m.chat, `Kamu Menjadi Tukang Sampah Dan Kamu Mendapat *20000* MONEY`, m)
    user.money += 20000
        user.lastnyampah = new Date * 0
