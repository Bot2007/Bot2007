...
let handler = async (m, { conn, command, args }) => {
  ...
  count = command.replace(/^buylimit/i, '') ? /all/i.test(count) ? Math.floor(global.DATABASE._data.users[m.sender].exp / xpperlimit) : parseInt(count) : args[0] ? parseInt(args[0]) : 1
...
handler.help = ['buy<jumlah limit>', 'buylimit <jumlah limit>', 'buyall']
handler.tags = ['xp']
