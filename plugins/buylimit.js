...
let handler = async (m, { conn, command, args }) => {
  ...
  if (global.DATABASE._data.users[m.sender].exp >= xpperlimit * count && global.DATABASE._data.users[m.sender].wang >= 1) {
    global.DATABASE._data.users[m.sender].exp -= xpperlimit * count
    global.DATABASE._data.users[m.sender].wang -= 1
    global.DATABASE._data.users[m.sender].limit += count
    conn.reply(m.chat, `-${xpperlimit * count} XP dan -RM1\n+ ${count} Limit`, m)
